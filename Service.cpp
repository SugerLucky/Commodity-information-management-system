...
//校验用户登录
bool Service::checkUser(string username, string password, int role) {
...
//查询商品
void Service::searchGoods(int id) {
	vector<Commodity> goodsInfo = FileStorage::loadAll<Commodity>();
	bool found = false;
	for (const auto& info : goodsInfo) {
		// 检查商品id是否匹配
